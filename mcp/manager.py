# mghier/hierarchical/mcp/manager.py (修正版)
...
class MCPManager:
    ...
    def get_tools_description(self) -> str:
        ...
        for tool_name, client in self.tool_to_client_map.items():
            # 查找该工具的详细信息
            tool_info = next((t for t in client.tools if t.get("name") == tool_name), None)
            description = tool_info.get('description', 'No description available.') if tool_info else 'No description available.'
            tool_details.append(f"- Tool: `{tool_name}` (from server: {client.name})\n  Description: {description}")
